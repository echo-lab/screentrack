...
public extension TreeMap {
    ...
    @objc public func tessellate(inRect rect: TreeMap.SystemRect) -> [TreeMap.SystemRect] {
        let rects = self.tessellate(weights: self.allWeights, inRect: rect.toYMTreeMapRect)
        return rects.map { $0.toSystemRect }
    }
}

internal extension TreeMap.Rect {
    internal var toSystemRect: TreeMap.SystemRect {
        return TreeMap.SystemRect(x: x, y: y, width: width, height: height)
    }
}

internal extension TreeMap.SystemRect {
    internal var toYMTreeMapRect: TreeMap.Rect {
        return TreeMap.Rect(x: Double(minX),
                            y: Double(minY),
                            width: Double(width),
